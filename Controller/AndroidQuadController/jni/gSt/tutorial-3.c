...
static void *app_function (void *userdata) {
  ...
  /* Build pipeline */
  data->pipeline = gst_parse_launch("udpsrc port=9000 caps=\"application/x-rtp, media=(string)video, clock-rate=(int)90000, encoding-name=(string)H264\" ! queue ! rtph264depay ! omxh264dec ! videoconvert ! autovideosink", &error);
  if (error) {
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
  ...
  jclass klass = (*env)->FindClass (env, "si/komp/quadcontroller/GStreamConnector");
