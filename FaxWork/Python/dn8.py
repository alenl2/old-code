...
def preisci(dir):
    ...
    for dat in os.listdir(dir):
        if os.path.isfile(dir+"/"+dat):
            if not dat in slo: slo[dat] = set()
            if dat in slo: slo[dat].add(dir)
        if os.path.isdir(dir+"/"+dat):
            gett = preisci(dir+"/"+dat)
            for i in gett:
                if not i in slo: slo[i] = set()
                if i in slo:
                    for x in gett[i]: slo[i].add(x)
    return slo

def isci(drevo, name):
    slo = {}
    for i in drevo:
        if name in i: slo[i] = drevo[i]
        if name in drevo[i]: slo[i] = drevo[i]
    return slo
...
def trojke(slo):
    ...
    sez = []
    for i in slo:
        for x in range(len(i)-2): sez.append(i[x:x+3])
        for j in sez:
            if j in i:
                if not j in vrni: vrni[str(j)] = set()
                if j in sez: vrni[str(j)].add(i)
    return vrni

def isci_hitro(dat, trojke, word):
    vrni = {}
    sez = []
    ssez = []
    for x in range(len(word)-2): sez.append(word[x:x+3])
    for ttr in trojke:
        for ttr2 in sez:
            if ttr2 == ttr: ssez.append(trojke[ttr])
    fs = {}
    for num, mn in enumerate(ssez): fs = mn.intersection(ssez[num])
    for vr in fs:
        if word in vr: vrni[vr] = dat[vr]
    return vrni

def isci_regex(dat, trojke, word):
    if word == "*":
        return dat
    vrni = {}
    sez = []
    ...
    for i in range(100):
        for vr in fs:
            x = re.search(tm.replace("?", "."), vr)
            if not x == None:
                if len(tm) == len(x.string): vrni[vr] = dat[vr]
        tm = word.replace("*", "?"*i)
    return vrni
