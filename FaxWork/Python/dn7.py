def zero(n):
    vrni = []
    for i in range(n): vrni.append(0)
    return vrni
...
def add(a, b):
    ...
    d = vector(b)
    while len(d) != len(c):
        if len(d) > len(c): c.append(0)
        if len(d) < len(c):d.append(0)
    for numa in enumerate(c):  vrni.append(numa[1] + d[numa[0]])
    return vrni
...
def iadd(a, b):
    ...
    while len(d) != len(c):
        if len(d) > len(c): c.append(0)
        if len(d) < len(c): d.append(0)
    for numa in enumerate(c):
        if numa[0] < len(a): a[numa[0]] = numa[1] + d[numa[0]]
        else: a.append(numa[1] + d[numa[0]])
...
def mul(a, b):
    vrni = []
    for numa in a: vrni.append(numa * b)
    return vrni

def imul(a, b):
    vrni = []
    for numa in enumerate(a): a[numa[0]] = numa[1] * b

def inner(a, b): 
    vrni = 0
    for i in enumerate(a):
        if i[0] < len(b): vrni += i[1] * b[i[0]]
    return vrni
...
def median(s):
    ...
    for sez in s:
        iii = 0
        for sez2 in s: iii += euclid(sez, sez2)/len(s)
        xx = iii
        if xx < tmp:
            tmp = iii
            vrni = sez
